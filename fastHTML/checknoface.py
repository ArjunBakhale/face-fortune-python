...
def main():
    ...
    if img_file_buffer is not None:
        ...
        with st.spinner("The spirits are studying your visage..."):
            # Annotate the face
            annotated_img, detection_result = annotate_face_in_pieces(img)
            
            if annotated_img is None:
                st.error("😕 The mystic forces could not detect a face in the image. Please try again with a clearer view of your face.")
            else:
                ...
                time.sleep(0.5)  # Add a delay for effect
                ...
                facial_features = "\n".join([f"{blendshape.category_name}: {blendshape.score}" for blendshape in detection_result.face_blendshapes[0]])
                fortune = generate_fortune(facial_features)
                st.markdown(f"""
